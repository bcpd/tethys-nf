...
def load_index_config(index_directory, logger):
    if not index_directory:
        logger.warning("No --index_directory provided; treating all merge artifact groups as required")
        return {}
    config_path = os.path.join(index_directory, "config.json")
    if not os.path.exists(config_path):
        logger.warning(f"Index config not found at {config_path}; treating all merge artifact groups as required")
        return {}
    with open(config_path) as handle:
        return json.load(handle)


def run_merge_step(logger, description, optional, callback):
    try:
        return callback()
    except FileNotFoundError as error:
        if optional:
            logger.warning(f"Skipping optional {description}: {error}")
            return None
        raise
...
def main(args=None):
    ...
    parser.add_argument("-d","--index_directory", type=str, help = "path/to/tethys index; config controls optional merge groups")
    ...
    config = load_index_config(opts.index_directory, logger)
    contains_genome_cluster_mapping = config.get("contains_genome_cluster_mapping", True)
    contains_pathways = config.get("contains_pathways", True)
    ...
    if opts.taxonomic_profiling_directory:
        ...
        for level in ["genomes", "genome_clusters"]:
            optional = level == "genome_clusters" and not contains_genome_cluster_mapping
            filepath = os.path.join(outdir, f"taxonomic_abundance.{level}.nc")

            def merge_taxonomy_level():
            ...
            run_merge_step(logger, f"{level} taxonomy", optional, merge_taxonomy_level)
    ...
    if opts.pathway_profiling_directory:
        ...
        for level, data_type, metric in chain(product(levels, abundance_data_types, metrics), product(levels, prevalence_data_types, ["number_of_reads"])):
            ...
            if any(illegal_conditions):
                continue
            optional = (
                (level == "genome_clusters" and not contains_genome_cluster_mapping)
                or (data_type == "pathway_abundances" and not contains_pathways)
            )
            ...
            run_merge_step(logger, f"{data_type}.{level}.{metric}", optional, merge_pathway_metric)
