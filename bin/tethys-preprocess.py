...
def main(args=None):
    ...
    logger = build_logger("tethys preprocess")
    ...
    if m not in {3,4}:
        raise IndexError("--input must have 3 or 4 tab-delimited columns")
        ...
        for id_genome, cds_filepath in tqdm(df_input["cds"].items(), total=number_of_genomes, desc="Extract feature sequences from CDS fasta"):
            for id_gene, seq in pyfastx.Fasta(cds_filepath, build_index=False):
                if id_gene in gene_to_features:
                    ...
                    print(">%s [id_genome=%s][features=%s]\n%s" % (id_gene, id_genome, feature_set, seq), file=f)
    ...
    if genome_to_genomecluster:
        for id_gene, feature_set in tqdm(gene_to_features.items(), desc=f"Writing feature mapping table [Genome_Clusters = True]"):
            if id_gene in cds_identifiers:
                id_genome = gene_to_genome[id_gene]
                id_genome_cluster = genome_to_genomecluster[id_genome]
                fields = [id_gene, feature_set, id_genome, id_genome_cluster]
                print(*fields, sep="\t", file=f_feature_mapping)  
                genomes_with_features.add(id_genome)
    else:
        for id_gene, feature_set in tqdm(gene_to_features.items(), desc=f"Writing feature mapping table [Genome_Clusters = False]"):
            if id_gene in cds_identifiers:
                id_genome = gene_to_genome[id_gene]
                fields = [id_gene, feature_set, id_genome]
                print(*fields, sep="\t", file=f_feature_mapping) 
                genomes_with_features.add(id_genome)
    ...
    logger.info(f"Completed preprocessing input data for tethys index: {opts.output_directory}")
    logger.info(f"Directory size of tethys preprocessing: {format_bytes(get_directory_size(opts.output_directory))}")
    logger.info(f"Directory structure of tethys preprocessing:\n{get_directory_tree(opts.output_directory, ascii=True)}")
    logger.info(f"Completed running tethys-preprocess: {opts.output_directory}")
