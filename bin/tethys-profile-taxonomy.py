...
def main(args=None):
    ...
    usage = f"{__program__} -1 forward.fq[.gz] -2 reverse.fq[.gz] -n sample_name -o project_directory --index_directory path/to/tethys_index/"
    epilog = "Leviathan"
    ...
    parser_io.add_argument("-s","--reads_sketch", type=str, help = "path/to/reads_sketch.sylsp (Cannot be used with -1/-2)")
    parser_io.add_argument("-n", "--name", type=str, required=True, help="Name of sample")
    parser_io.add_argument("-o","--project_directory", type=str, default="tethys_output/profiling/taxonomy", help = "path/to/project_directory")
    ...
    parser_sylph_sketch.add_argument("--sylph_k", type=int, choices={21,31}, default=31,  help="Sylph | k (21 or 31). [Default: 31]")
    parser_sylph_sketch.add_argument("--sylph_minimum_spacing", type=int,  default=30,  help="Sylph | Minimum spacing between selected k-mers [Default: 30]")
    parser_sylph_sketch.add_argument("--sylph_subsampling_rate", type=int, default=200,  help="Sylph | Subsampling rate. [Default: 200]")
    parser_sylph_sketch.add_argument("--sylph_sketch_options", type=str, default="", help="Sylph | More options for `sylph sketch` [Default: '']")

    parser_sylph_profile = parser.add_argument_group('Sylph profile arguments')
    parser_sylph_profile.add_argument("--sylph_profile_minimum_ani", type=float, default=95, help="Sylph profile | Minimum adjusted ANI [Default: 95]")
    parser_sylph_profile.add_argument("--sylph_profile_minimum_number_kmers", type=int, default=50, help="Sylph profile | Min number of sampled k-mers [Default: 50]")
    parser_sylph_profile.add_argument("--sylph_profile_minimum_count_correct", type=int, default=3, help="Sylph profile | Min k-mer multiplicity for correction [Default: 3]")
    parser_sylph_profile.add_argument("--sylph_profile_options", type=str, default="", help="Sylph profile | More options for `sylph profile` [Default: '']")
    ...
    if not opts.sylph_executable:
        sylph_in_path = shutil.which("sylph")
        opts.sylph_executable = sylph_in_path or os.path.join(bin_directory, "sylph")
    if not os.path.exists(opts.sylph_executable):
        raise FileNotFoundError(f"sylph executable not found: {opts.sylph_executable}")
    ...
    if input_reads_format == "paired":
        ...
        logger.info(f"Using provided reads sketch: {opts.reads_sketch}")
        if opts.reads_sketch is None:
            raise ValueError("Provide paired reads or a Sylph sketch")
    ...
    for id_genome, data in tqdm(genome_to_data.items(), "Getting filepaths associated with genomes"):
        ...
        genome_to_genomecluster[id_genome] = data.get("id_genome_cluster")
    del genome_to_data
    ...
    if any(v is not None for v in genome_to_genomecluster.values()):
