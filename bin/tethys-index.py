...
def main(args=None):
    ...
    parser_io.add_argument("-f","--fasta", type=str,  help = "path/to/cds.fasta")
    ...
    logger = build_logger("tethys index")
    ...
    if not opts.no_check:
        ...
        if genomes_with_filepaths == genomes_with_features:
            logger.info("All genomes in --genomes are in --feature_mapping")
        else:
    ...
    if opts.update_with_genomes:
        if not opts.genomes:
            parser.error("--genomes is required when --update_with_genomes is specified")
        check_salmon_index(os.path.join(opts.index_directory, "salmon_index"))
        if not opts.fasta or not opts.feature_mapping:
            parser.error("--fasta and --feature_mapping are required when --update_with_genomes is not specified")
    ...
    if not os.path.exists(opts.salmon_executable):
        raise FileNotFoundError(f"salmon executable doesn't exist: {opts.salmon_executable}")
    ...
    if not os.path.exists(opts.sylph_executable):
        raise FileNotFoundError(f"sylph executable doesn't exist: {opts.sylph_executable}")
    ...
    if opts.pathway_database:
        ...
        if not opts.pathway_database_downloader_executable:
            downloader_in_path = shutil.which("build-pathway-database.py")
            opts.pathway_database_downloader_executable = downloader_in_path or os.path.join(bin_directory, "build-pathway-database.py")
        if not os.path.exists(opts.pathway_database_downloader_executable):
            raise FileNotFoundError(
                f"KEGG Pathway Profiler's `build-pathway-database.py` executable doesn't exist: {opts.pathway_database_downloader_executable}."
            )

    if all([os.path.exists(opts.index_directory), not opts.update_with_genomes]):
        raise FileExistsError(f"--index_directory {opts.index_directory} already exists. Use --update_with_genomes or remove it")
    ...
    if not opts.update_with_genomes:
        ...
        if opts.pathway_database:
            logger.info("Copying pathway database to index")
            ...
            if config["feature_type_is_kegg_ortholog"]:
                ...
                cmd_kegg = run_kegg_pathway_downloader(
                    logger=logger,
                ...
                logger.warning("No pathway DB provided and features not KOs; pathway profiling will be limited")
        ...
        logger.info("Updating genome database with filepaths")
    ...
    if config.get("contains_genome_filepaths", False):
    ...
    logger.info(f"Completed building tethys index: {opts.index_directory}")
    logger.info(f"Directory size of tethys index: {format_bytes(get_directory_size(opts.index_directory))}")
    logger.info(f"Directory structure of tethys index:\n{get_directory_tree(opts.index_directory, ascii=True)}")
    logger.info(f"Completed running tethys-index: {opts.index_directory}")
